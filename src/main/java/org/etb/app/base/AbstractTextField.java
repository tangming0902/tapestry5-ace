 ...
 * text field. Most of the hooks for user input validation are in this class.
 * <p/>
 * In particular, all subclasses support the "toclient" and "parseclient"
 * events. These two events allow the normal {@link Translator} (specified by
 * the translate parameter, but often automatically derived by Tapestry) to be
 * augmented.
 * <p/>
 * If the component container (i.e., the page) provides an event handler method
 * for the "toclient" event, and that handler returns a non-null string, that
 * will be the string value sent to the client. The context passed to the event
 ...
 * passed the string provided by the client, and may provide a non-null value as
 * the parsed value. Returning null allows the normal translator to operate. The
 * event handler may also throw {@link org.apache.tapestry5.ValidationException}.
 */
@Events({ EventConstants.TO_CLIENT, EventConstants.VALIDATE,
		EventConstants.PARSE_CLIENT,
		EtbEventConstants.SYNCHRONIZE + "额外加入的事件，用于组合组件的数据同步" })
public abstract class AbstractTextField extends AbstractField {
	 ...
	 * translator may be provided to convert between client side and server side
	 * representations. If not bound, a default binding is made to a property of
	 * the container matching the component's id. If no such property exists,
	 * then you will see a runtime exception due to the unbound value parameter.
	 */
	 ...
	 * translation). The validate binding prefix is generally used to provide
	...
	/**
	 * Provider of annotations used for some defaults. Annotation are usually
	 * provided in terms of the value parameter (i.e., from the getter and/or
	 * setter bound to the value parameter).
	 * 
	...
	final AnnotationProvider defaultAnnotationProvider() {
		return new AnnotationProvider() {
			public <T extends Annotation> T getAnnotation(
				...
				return resources.getParameterAnnotation("value",
						annotationClass);
			}
		};
	}
	...
	void begin(MarkupWriter writer) {
		...
		// If this is a response to a form submission, and the user provided a
		// value.
		// then send that exact value back at them.

		if (value == null) {
			// Otherwise, get the value from the parameter ...
			// Then let the translator and or various triggered events get it
			// into
			// a format ready to be sent to the client.

			value = fieldValidationSupport.toClient(this.value, resources,
					translate, nulls);
		}
	 ...
	 * {@linkplain AbstractField#getControlName() controlName} and
	 * {@linkplain AbstractField#getClientId() clientId} properties will already
	 * have been set or updated.
	 * <p/>
	 * Generally, the subclass will invoke
	 * {@link MarkupWriter#element(String, Object[])}, and will be responsible
	 * for including an {@link AfterRender} phase method to invoke
	 * {@link MarkupWriter#end()}.
	 * 
	 * @param writer
	 *            markup write to send output to
	 * @param value
	 *            the value (either obtained and translated from the value
	...
	protected void processSubmission(String controlName) {
		...
		try {
			...
			resources.triggerEvent(EtbEventConstants.SYNCHRONIZE,
					new Object[] { translated }, null);
	...
	// /**
	// * Looks for a {@link org.apache.tapestry5.beaneditor.Width} annotation
	// and, if present, returns its value as a
	// * string.
	// *
